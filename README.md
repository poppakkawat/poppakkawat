...
A small, **zero-dependency** companion tool that scrapes recent posts from a
**single** Facebook account and saves them as Markdown or JSON. It reads the
lightweight mobile site (`mbasic.facebook.com`), parses each post's author,
text, timestamp, permalink, and outbound links, and follows the timeline's
...
Run the tests with `python3 tests/test_fbscrape.py` (or `pytest`).
